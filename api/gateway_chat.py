...
def _run_gateway_chat_streaming(
    ...
    *,
    ...
    def put_gateway_event(event, data):
        ...
        # path raised). Upstream #7272: snapshot-only events carry no event id,
        # but snapshot-capable queues still accept (and rely on) id-bearing
        # items for reconnect cursors, so keep the 3-tuple shape.
        try:
