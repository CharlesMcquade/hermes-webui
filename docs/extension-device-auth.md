...
Chrome emits an extension Origin on POST but can omit it on GET. A GET/HEAD with no Origin must carry `X-Hermes-Extension-Id` matching the bearer-bound extension identity. A supplied Origin must always be the matching extension Origin, even when the identity header is correct. Originless unsafe requests are rejected. The ID is a public-client identifier, **not cryptographic browser attestation**; possession of a stolen bearer remains sufficient for a non-browser attacker with that public ID.
