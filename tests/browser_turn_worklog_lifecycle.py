...
from browser_conversation_lifecycle import (
    ...
    _capture_page_errors, _start_webui_server,
...
def snapshot(page):
    return page.evaluate("""() => {
      ...
      const rows = Array.from(turn.querySelectorAll('[data-anchor-scene-row="1"]')).map(el => ({
        ...
        noDetail: !!el.querySelector('.tool-card-no-detail'),
            ...
            with sync_playwright() as playwright:
                ...
                if origin == 'turn_worklog' and scenario == 'turn-worklog':
                    assert page.evaluate("""() => {
                      for (const settled of [false, true]) {
                        const row = _anchorSceneNodeForRow({
                          role:'tool', row_id:'fixture-expandable-tool',
                          tool:{name:'terminal', args:{command:'printf fixture'}, snippet:'fixture result', done:true},
                        }, {settled});
                        document.body.appendChild(row);
                        try {
                          const card=row.querySelector('.tool-card');
                          const header=row.querySelector('.tool-card-header');
                          if (!card || !header || !row.querySelector('.tool-card-detail') ||
                              header.getAttribute('role') !== 'button' ||
                              header.getAttribute('tabindex') !== '0' ||
                              header.getAttribute('aria-expanded') !== 'false' ||
                              card.classList.contains('open')) return false;
                          header.dispatchEvent(new KeyboardEvent('keydown', {key:'Enter', bubbles:true}));
                          if (!card.classList.contains('open') || header.getAttribute('aria-expanded') !== 'true') return false;
                          header.click();
                          if (card.classList.contains('open') || header.getAttribute('aria-expanded') !== 'false') return false;
                        } finally { row.remove(); }
                      }
                      return true;
                    }"""), 'expandable live/settled tool details must start closed and remain keyboard-accessible'
                ...
                if origin == 'turn_worklog':
                    ...
                    if scenario == 'turn-worklog':
                        assert [row['id'] for row in live['rows'] if row['role'] == 'prose'] == [
                            'fixture-progress-0', 'fixture-progress-1', 'fixture-progress-2'
                        ], live
                    ...
                    for row in live['rows']:
                        ...
                        if row['role'] == 'tool':
                            # Read-only tools in this production renderer intentionally
                            # have no detail/chevron. Expandable tools start closed.
                            if not row['noDetail']:
                                assert any(item['expanded'] == 'false' for item in row['disclosure']), live
                            assert not row['cardOpen'], live
