...
def test_fallback_notice_persisted_on_assistant_message_before_save():
    ...
    assert stamp_idx > turn_duration_idx and stamp_idx < turn_duration_idx + 2400, (
...
def test_stream_scoped_fallback_notices_dict_exists():
    ...
    # it skips popping when _cancel_claimed is set.
    cleanup_idx = src.find("not _fb_entry.get('_cancel_claimed')")
    assert cleanup_idx != -1, (
        "_run_agent_streaming's finally block must check _cancel_claimed "
        "and skip popping when cancel has claimed the notice."
    )
